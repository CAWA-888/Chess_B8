public class Horse extends ChessPiece {
    ...
    public boolean canMoveToPosition(ChessBoard chessBoard, int line, int column, int toLine, int toColumn) {
        if (checkPos(line) && checkPos(column) && checkPos(toLine) && checkPos(toColumn)) {
            if (line != toLine && column != toColumn && (chessBoard.board[toLine][toColumn] == null || // проверь эту лошадь
                    !chessBoard.board[toLine][toColumn].color.equals(this.color)) &&                   // не могу съехать
                ...
                for (int i = 0; i < positions.length; i++) {
                    if (positions[i][0] == toLine && positions[i][1] == toColumn)
                        return true;  // проверьте, что для строки и столбца
                }                                                                               // in positions
            }
        } else return false;
        return false;
